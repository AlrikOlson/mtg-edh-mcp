...
- [ ] **Release · Localhost hardening — loopback-only sidecar + auth posture decision** — Refresh discovery (think:166): the packaged app runs the engine as a streamable-HTTP sidecar on every user's machine, and the transport has no authentication (open signal be878f98, raised June 29 — escalates from theoretical to real at public scale). Work: (a) VERIFY the sidecar binds 127.0.0.1 only (currently unverified — if it binds 0.0.0.0, any LAN peer can read/mutate a user's decks); (b) decide + implement the localhost auth posture: accept-loopback-only is a defensible v1 answer, a per-launch shared-secret header is the stronger one (the GUI already injects headers via x-mcp-principal machinery); (c) note the posture in the security section of the docs. Scope is the DESKTOP sidecar only — hosted-HTTP auth stays out of scope for a portfolio release (that half of signal be878f98 stays open).
  - deps: gui-packaging
  - acceptance: The sidecar listener is verified bound to 127.0.0.1 (not 0.0.0.0) in the packaged app
  - acceptance: A recorded decision on localhost auth (loopback-only vs shared secret), implemented if secret
  - acceptance: Docs state the security posture
