...
- [ ] **GUI · Design tokens + component gallery (the Storybook analog)** — Phase G (think:114), /craft §B. The design grammar this project will own — NOT a default-component-library look. A token system as CSS custom properties (color incl. the WUBRG mana palette, spacing scale, type scale, radii, elevation, motion), light + dark, consumed by every component (Dioxus styles with HTML+CSS; built-in Tailwind available). Plus a component-gallery route (the Storybook 'everything in a gallery' analog for Dioxus): each primitive + object component rendered in all states for visual review and Playwright scrutiny. Establish the brand grammar (a senior-designer MTG aesthetic, mana-color-aware, not AI-slop gradients/emoji).
  ...
  - acceptance: A /gallery route renders every primitive + object component in its states (the Dioxus Storybook analog)
  - acceptance: Components consume tokens only (no one-off magic values); the look is a deliberate MTG grammar, not a default template
- [ ] **GUI · App shell — routing, signals app-state, connection + sidecar** — Phase G (think:114). The window/layout/navigation shell + global app state. Dioxus router for the object screens; signals-based app state holding the MCP client handle, the active deck_id + principal/session, and connection status (connecting/ready/upstream-degraded). Desktop sidecar wiring: optionally spawn the node mtg-edh-mcp `bin` as a child and connect locally, with a clear 'engine offline' state + retry (graceful, mirroring the engine's UPSTREAM_UNAVAILABLE ethos). This is the frame every object screen mounts into.
  - deps: gui-mcp-client, gui-design-tokens
  - acceptance: Router + layout shell with navigation between the object screens; light/dark honored
  - acceptance: Signals app-state exposes the MCP client, active deck/session, and a connection status surfaced in the UI
  - acceptance: Desktop can spawn + connect to the engine (sidecar) and shows a clear offline/retry state when it can't
- [ ] **GUI · Card object — search, detail, printings, pricing** — Phase G (think:114), OOUX object #1: the Card. ONE canonical Card component (recognizable, connected, valuable — manifesto #9) reused everywhere a card appears: search results, deck rows, recommendations. Screens: card_search with the Scryfall-grammar query box + owned_only toggle (collection), the Card detail (card_get / card:// — oracle text, type, mana cost, color identity, roles, legality), card_printings + default/cheapest pricing. Consistent CTAs on the Card object (add-to-deck, add-to-collection, view printings) wherever it renders.
  - deps: gui-app-shell
  - acceptance: A single Card component renders identically in search, deck, and recommendation contexts with consistent CTAs (OOUX fidelity)
  - acceptance: card_search works with the grammar query, ordering, pagination, and the owned_only toggle
  - acceptance: Card detail shows oracle/type/cost/identity/roles/legality + printings + default & cheapest price
- [ ] **GUI · Deck object — editor, commander, validate, import/export, snapshots** — Phase G (think:114), OOUX object #2: the Deck. Deck list + create; the decklist editor (deck_add/deck_remove with the inline pre-check verdict, deck_set_commander / deck_set_companion with name-or-id); the live validate_deck verdict (errors vs warnings vs the any-number/companion notes) surfaced as the authoritative gate; deck_import (Moxfield/Archidekt/MTGO/Arena/plaintext with the unresolved-lines report) + deck_export; snapshot/diff/restore (try-a-variant, compare, roll back). Optimistic-version conflict handling (expected_version). Reuses the Card component for rows.
  - deps: gui-app-shell, gui-card
  - acceptance: Create → edit (add/remove/commander/companion) → validate round-trips; the validate verdict (errors/warnings/notes) is shown as the gate
  - acceptance: Import (with unresolved-lines report) + export both work; snapshot → mutate → diff → restore works
  - acceptance: Version conflicts surface gracefully (expected_version); the Deck reuses the canonical Card component
- [ ] **GUI · Collection object — owned cards + build-from-collection** — Phase G (think:114), OOUX object #3: the Collection. Manage owned cards (collection_set/add/get/clear; name-or-id with the unresolved report); the collection:// resource view; the owned_only filter wired into card_search; and the build-from-collection surface — budget_plan with use_collection showing acquire_usd ('cost to finish from what you own') vs full min-buy. The moat feature made visible.
  - deps: gui-app-shell, gui-card
  - acceptance: Add/remove/clear owned cards; the owned set is visible and editable; unresolved entries reported
  - acceptance: card_search owned_only toggle filters to the collection from the UI
  - acceptance: budget_plan use_collection surfaces acquire_usd (build-from-what-you-own) alongside the full min-buy
- [ ] **GUI · Analysis views — curve / composition / mana / roles / goldfish** — Phase G (think:114), the analysis surface over a Deck (the differentiated moat made visual). Chart-friendly views for analyze_curve (MV distribution), analyze_composition (by type + functional role), analyze_stats (avg MV, color pips, totals + min-buy), analyze_mana_base (per-color sources, untapped/tapped, fixing density, under-supported flags), analyze_role_coverage (vs target bands → gaps), and simulate_deck (the Monte Carlo goldfish — keepable %, mulligan/DOA, lands-by-turn, turn-to-first-spell, deterministic by seed). Real, readable data viz — not decorative.
  - deps: gui-app-shell, gui-deck
  - acceptance: Curve/composition/stats/mana/role-coverage render as readable, accurate charts bound to the live deck
  - acceptance: simulate_deck goldfish stats display with a seed control; identical seed → identical view (determinism honored)
  - acceptance: Under-supported colors / role gaps are visibly flagged, not buried
- [ ] **GUI · Meta surface — bracket, budget, recommendations, combos** — Phase G (think:114), the enrichment/meta surface. The power-level bracket verdict + pushers (meta_classify_bracket — Game Changers / fast mana / MLD / two-card combos / extra turns, the P11 work made visible); budget_plan (reprint savings, cost drivers, target gap, owned acquire); meta_recommendations / meta_missing_staples / meta_budget_swaps / meta_combos / meta_commander_profile / meta_themes; and meta_deck_summary as the one-call overview. All EDHREC/Spellbook-backed → must show the graceful UPSTREAM_UNAVAILABLE degraded state (not a crash) when enrichment is down.
  - deps: gui-app-shell, gui-deck
  - acceptance: Bracket verdict + pushers shown clearly (incl. combos/extra-turns); meta_deck_summary as a one-call overview
  - acceptance: Budget (reprint/cost-drivers/target/owned-acquire) + recommendations/missing-staples/swaps/combos render with source+confidence where present
  - acceptance: Enrichment-down (UPSTREAM_UNAVAILABLE) degrades to a clear advisory state, never a crash
- [ ] **GUI · Scrutiny gate — Playwright (web target), light/dark, anti-AI-slop** — Phase G (think:114), the /craft §B + /gui-scrutiny verify gate adapted for Dioxus. Dioxus compiles to web (WASM) too: build the WEB target and drive the running app with the Playwright MCP — empirical review in LIGHT + DARK, visual AND mechanical (bespoke DOM assertions, not vibes), plus axe a11y. RELIGIOUSLY anti-AI-slop: no generic templated layouts, unmotivated gradients/shadows, emoji-as-UI, lorem/placeholder/fake metrics, default-component-library look — it must read as a senior product designer's work grounded in this project's MTG grammar (gui-design-tokens). OOUX fidelity check: the same object (Card/Deck) renders through one component with consistent CTAs everywhere (manifesto #9). Wire the gallery-route stories into the gate as regression coverage.
  - deps: gui-card, gui-deck, gui-collection, gui-analysis, gui-meta
  - acceptance: The web target is driven by Playwright in light + dark with mechanical DOM assertions (not screenshots-only) + axe a11y
  - acceptance: An anti-AI-slop pass is recorded: no templated/gradient/emoji/lorem/fake-metric slop; the look matches the project's design grammar
  - acceptance: OOUX fidelity verified: each object renders through one component with consistent CTAs across screens
