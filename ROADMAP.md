...
- [x] **Ergonomics · Wave 3 — MCP prompts (build/tune/budget recipes) + agent cookbook** — Make the workflow a first-class server artifact: three MCP prompts (build_commander_deck {commander, theme?, budget_usd?}, tune_deck {deck_id}, fit_budget {deck_id, target_usd}) in new src/server/prompts.ts, registered when index+deckStore exist; each returns a recipe message teaching the tool loop and the failed[]/vitals conventions, referencing only existing tools (lint-checked like FLOW lines). New docs/AGENT-COOKBOOK.md (3-call build loop, error recovery, annotation map) linked from README. prompts.test.ts proves list/get + tool-name validity on the wire.
  ...
  - acceptance: npm gates green
- [x] **Security · Verify CVE-2026-25536 (SDK cross-client leak) doesn't touch our stateless HTTP mode** — CVE-2026-25536: cross-client data leak in @modelcontextprotocol/sdk when a single McpServer is shared across clients in stateless deployments (miggo.io advisory). Our http.ts creates a FRESH server per POST (the pinned think:134 pattern), which is likely the exact mitigation — but verify, don't assume: (1) confirm the affected SDK version range vs our ^1.29.0 and bump if a patched release exists; (2) confirm no code path shares one McpServer across principals; (3) add a regression test (two principals, interleaved POSTs, assert zero cross-principal leakage — extend multitenancy.test.ts). Small chunk; stdio mode is single-client and unaffected.
  - acceptance: SDK version confirmed unaffected or bumped past the patched release
  - acceptance: explicit test proves no cross-principal leakage over stateless HTTP
...
- [ ] **MCP · Publish to the official MCP Registry (+ GitHub MCP Registry)** — The official MCP Registry (modelcontextprotocol.io/registry) is the standard OSS discoverability path — publish server.json metadata for the engine (stdio launch via the mtg-edh-mcp bin; npm package identity), and surface in the GitHub MCP Registry (github.blog 2025-09-16). Cheap, high-visibility portfolio move that showcases the agent-first surface (41 template-described tools, annotations, prompts, AGENT-COOKBOOK). Requires deciding the npm publish story first (the package is currently not published) — that decision gates this chunk.
  - acceptance: server.json in-repo and validated against the registry schema
  - acceptance: entry live in the official registry (and GitHub registry) pointing at the repo/npm package
  - acceptance: README badges/links updated
...
- [ ] **MCP · Migrate to TS SDK v2 + spec 2026-07-28 (after v2 stabilizes)** — TS SDK v2 ships alongside the 2026-07-28 spec (July 28, 2026 — typescript-sdk README): the monolithic @modelcontextprotocol/sdk splits into @modelcontextprotocol/server (+node); v1.x gets bug/security fixes for ≥6 months. The 2026-07-28 spec brings a STATELESS PROTOCOL CORE (our fresh-server-per-POST HTTP design is exactly this shape — validation, not rework), an Extensions process, and full JSON Schema for tools (upgrade path for our outputSchema trio and eventually all 41). Wait for v2 stable + a point release, then migrate: registry.ts is the single choke point (ToolDefinition config passes through), so the blast radius is contained. Watch items: descriptions/prompts lint pins, zod version coupling, rmcp client compatibility (protocol-version negotiation).
  - acceptance: engine on @modelcontextprotocol/server v2 stable, negotiating spec 2026-07-28
  - acceptance: all wire-lint pins (41 tools, annotations, outputSchema, prompts) still green
  - acceptance: gui rmcp round-trips green (protocol negotiation compatible)
- [ ] **MCP · Adopt spec async tasks for data_ingest (+ evaluate elicitation)** — Spec 2025-11-25 standardized async tasks and elicitation (changelog: modelcontextprotocol.io/specification/2025-11-25/changelog; WorkOS calls it 'the largest set of changes since launch'). Our data_ingest is a hand-rolled start+poll loop (IngestRunner + data_status) — once the SDK v2 exposes spec-native tasks, migrate data_ingest to them (harnesses then track the long-running ingest natively instead of polling). Separately evaluate elicitation for AMBIGUOUS_NAME resolution (server asks the user to pick a candidate) — only if major harnesses actually surface elicitation by then; keep failed[]+candidates as the fallback contract either way.
  - deps: mcp-sdk-v2-migration
  - acceptance: data_ingest exposed as a spec-native task with progress; data_status retained for compat
  - acceptance: elicitation adoption decision recorded with harness-support evidence (adopt or explicitly decline)
  - acceptance: gates green incl. gui round-trips
- [ ] **Strategic · Extract the analysis engine as a reusable library / hosted-API surface** — Business-intel bet (think:107-108; premise REVISED by refresh think:159-161; MCP-landscape update think:205). The moat is the PURE analysis core — src/validate/* (core/commander/companion rules), src/analyze/* (roles, stats, mana, sim, budget), src/meta/bracket.ts — all IO-free, deterministic, offline-testable. COMPETITIVE PREMISE UPDATE (2026-07-01): the 'no AI-native competitor' claim is stale — an AI-MTG-assistant category has formed (ManaTap/manatap.ai: deck-aware AI chat + mulligan testing; KrakenTheMeta; MTG Agents; an iOS 'MTG Deck Builder AI' app). 'AI chat over a deck' is now table stakes; what the entrants LACK is a deterministic engine under the LLM (seeded Monte Carlo sim, real rules validation, bracket classification, budget planning) — which sharpens this bet: package the engine so it can be licensed/embedded by exactly those products, or power a hosted API. Consumer willingness-to-pay stays near zero (Moxfield/Archidekt are free + $1/mo Patreon tiers — grimdeck.com Mar 2026), so the library/B2B route remains the only credible monetization path. MCP-LANDSCAPE UPDATE (2026-07-04, think:205): the hosted-API leg strengthened on two axes — (a) the 2026-07-28 spec RC makes a STATELESS PROTOCOL CORE the MCP norm, and our fresh-server-per-POST HTTP transport is already exactly that shape (the 'hosted MCP endpoint' path needs auth added, not architecture changed); (b) the six-chunk ergonomics program (ergo-resolve…ergo-prompts) means the MCP surface itself is now a differentiated product artifact: 41 template-described tools, annotations, workflow prompts, AGENT-COOKBOOK — an embeddable, agent-first API story no MTG entrant has. Note the GUI already consumes the engine via MCP (a second surface exists); this chunk is about a TRANSPORT-INDEPENDENT package: (a) publish src/{validate,analyze,meta} as a standalone npm library with a clean public API; and/or (b) a thin hosted HTTP/JSON or hosted-MCP API. Prereq decision: business-vs-portfolio (the human's call — reaffirmed PORTFOLIO at think:164). Time-sensitivity: the entrants are shipping now; the whitespace ('the deterministic engine layer for MTG AI tools') is open but closing.
